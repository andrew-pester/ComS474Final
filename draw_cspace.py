...
def draw_cspace(ax, cspace, obstacles, tick_step=[1, 1, 1]):
    ...
    for obs in obstacles:
        ax.plot([v[0] for v in obs], [v[1] for v in obs],[v[2] for v in obs],  "r-", linewidth=3)
    ...
    ax.set_zticks(
        range(math.ceil(cspace[2][0]), math.floor(cspace[2][1]) + 1, tick_step[2])
    )
